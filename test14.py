"""
test14.py -- QLoRA (4-bit) experiments on RTX 4070 (12 GB VRAM).

Mirrors test13-a config but adapted for 12 GB:
  - BitsAndBytesConfig (nf4 / double-quant / bf16 compute) reduces base model
    footprint to ~1 GB, leaving room for activations + LoRA gradients.
  - batch_size=1 + gradient_accumulation=8 (effective batch = 8, same as test13).
  - prepare_model_for_kbit_training() called before LoRA is applied.

Experiments (sequential, RTX 4070 12 GB VRAM):
  test14-a  Qwen3-1.7B QLoRA  schema_sorted  aug_v2  r=16 alpha=16  2048  3ep
  test14-b  identical to test14-a but alpha=32  (isolates alpha effect)

Baseline: Method 3  LB=0.4415  Table=0.5538  Column=0.3292
test11:              LB=?       Table=?       Column=0.3629   (r=16 alpha=16 len=2048)

Adapters → ./adapters/test14-a/   ./adapters/test14-b/
Preds    → test14-a-preds.json    test14-b-preds.json
Log      → test14-log.txt

Usage (inside tmux on RTX 4070 machine):
    conda run -n cse234 python test14.py
"""
...
BNB_CONFIG = BitsAndBytesConfig(
    ...
    bnb_4bit_compute_dtype=torch.bfloat16, # fast bf16 matmul on compute
)
...
EXPERIMENTS = [
    {
        'tag':         'test14-a',
        'adapter_dir': './adapters/test14-a',
        'preds_file':  './test14-a-preds.json',
        'lora_r':      16,
        'lora_alpha':  16,    # alpha == r (no effective scaling)
        'lr':          2e-4,
        'epochs':      3,
        'batch_size':  1,     # 12 GB VRAM constraint
        'grad_accum':  8,     # effective batch = 8
        'max_length':  2048,
    },
    {
        'tag':         'test14-b',
        'adapter_dir': './adapters/test14-b',
        'preds_file':  './test14-b-preds.json',
        'lora_r':      16,
        'lora_alpha':  32,    # alpha = 2×r (Method 3 default) — isolates alpha effect
        'lr':          2e-4,
        'epochs':      3,
        'batch_size':  1,
        'grad_accum':  8,
...
def run_training(exp: dict, train_data: list) -> bool:
    ...
    try:
        ...
        sft_cfg = SFTConfig(
            ...
            save_total_limit=1,
            report_to="none",
            optim="paged_adamw_8bit",   # paged optimizer reduces VRAM spikes
            warmup_ratio=0.05,
...
def run_inference(exp: dict, val_items: list) -> bool:
    ...
    try:
        ...
        # Load base in 4-bit (same config as training) then apply adapter
