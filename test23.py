"""
test23.py -- Long-schema truncation fix on RTX 4070 (12 GB VRAM, bf16).

ROOT CAUSE of test22's 0.4444 (vs test20-a 0.4990):
  test22 produced 22 EMPTY predictions ({}), each scoring 0. They cluster
  exactly on the databases whose serialized schema exceeds max_length=2048:
    NTSB (~2.8k tok)          → 13 empties
    SBODemoUS-* (3k–6k tok)   →  7 empties
  while every small-schema DB (< ~2k tok) had ZERO empties.

  Mechanism: with completion_only_loss=True and max_length=2048, the long
  NTSB/SBO schema fills (and overruns) the window, so the assistant JSON
  answer is TRUNCATED OFF the end → those examples carry NO loss signal →
  the model never learns to emit JSON for long-schema DBs → at inference it
  degenerates to unparseable/empty output. test22 made this worse than
  test20-a because CED-v3 removed ~1193 long NTSB/NYSED CED examples, so the
  model saw even fewer long prompts and tipped from "over-predict (non-empty,
  partial credit)" to "empty (zero credit)".

THE FIX (single method): raise max_length 2048 → 3072 so long-schema
  completions are retained during training, plus a guaranteed non-empty
  keyword fallback at inference so no prediction can ever score 0 (covers the
  giant SBO schemas that still exceed 3072). Training data unchanged from
  test22 (CED-v3 + hardcoded examples).

  Why 3072 and not 4096: the LM-head logits tensor [1, seq, vocab=151936] is
  the largest training activation and scales linearly with seq. At seq=4096
  the realistic peak on 12 GB is ~9-10 GB (risky with fragmentation); at 3072
  it is ~7-8 GB. 3072 still covers NTSB (13 of the 22 empties — the dominant
  cluster) plus SBO-General/Reports/Service; the few giant SBO schemas
  (Inventory/Finance/Banking, >4k tok, 0-2 val Qs each) are caught by the
  fallback. attn_implementation="sdpa" is pinned so attention memory is O(seq)
  not the eager O(seq^2) score matrix.

RTX 4070 (12 GB) settings kept from test22:
  ...
  - gradient_checkpointing=True (keeps activations small at seq=3072)

Memory at seq=3072, batch=1, bf16 + grad-ckpt + sdpa:
  - weights ~3.4 GB + LoRA/Adam ~0.2 GB + checkpointed activations ~0.35 GB
  - logits + fp32 CE upcast + grad ~2.5-3 GB + CUDA/frag ~1-2 GB
  - realistic peak ~7-8 GB on 12 GB  ✓

Hardware: RTX 4070 (12 GB VRAM)
Adapter → ./adapters/test23/
Preds   → test23-preds.json
Log     → test23-log.txt
"""
...
# Longer sequences fragment the allocator more; expandable_segments keeps
# peak reservation down (and avoids the NVML assert seen on MIG slices).
...
EXP = {
    'tag':          'test23',
    'adapter_dir':  './adapters/test23',
    'preds_file':   './test23-preds.json',
    ...
    'max_length':   3072,   # was 2048 — long NTSB completions were truncated.
                            # 3072 (~real tok) covers NTSB (13 of 22 empties) and
                            # most SBO; keeps 12 GB peak ~7-8 GB. Giant SBO
                            # (Inventory/Finance/Banking >4k tok) still truncate
                            # but the keyword fallback covers them.
    'hypothesis': (
        'Long-schema truncation fix: max_length 2048->3072 so NTSB/SBODemoUS '
        'JSON completions are no longer truncated during training (test22 had '
        '22 empty predictions, all on >2048-token schemas, each scoring 0). '
        'Plus a guaranteed non-empty keyword fallback at inference. Training '
        'data unchanged from test22 (CED-v3 + hardcoded).'
    ),
}
...
# ── Non-empty inference fallback ──────────────────────────────────────────────

_FB_STOP = {
    'the','and','for','that','with','from','show','list','count','number',
    'each','all','have','has','are','was','were','which','what','how','many',
    'their','they','this','these','those','where','when','who','whom','than',
    'into','per','by','of','in','on','to','a','an','is','be','as','at','or',
    'display','give','find','get','average','total','sum','max','min','most',
    'least','value','values','ignore','should','codes','code','lookup','name',
}
...
def keyword_fallback(question, schema):
    """Guaranteed non-empty guess so a question never scores 0.

    Picks the table whose name+column tokens overlap the question the most,
    then the columns of that table whose tokens overlap the question (or the
    whole table's first few columns if none overlap). Reads only from the
    schema, so every identifier is valid. Only invoked when the model output
    parses to empty (e.g. a schema still longer than max_length)."""
    qtok = _tok(question)
    if not qtok or not schema:
        # last resort: smallest table, its first column
        t = min(schema, key=lambda x: len(schema[x])) if schema else None
        return {t: schema[t][:1]} if t and schema.get(t) else {}

    def col_score(table):
        hits = sum(1 for c in schema[table] if _tok(c) & qtok)
        name_hit = 2 if _tok(table) & qtok else 0
        return hits + name_hit

    ranked = sorted(schema, key=col_score, reverse=True)
    best = ranked[0]
    if col_score(best) == 0:
        best = min(schema, key=lambda x: len(schema[x]))

    cols = [c for c in schema[best] if _tok(c) & qtok][:6]
    if not cols:
        cols = schema[best][:3]
    return {best: cols}
...
def run_training(train_data):
    ...
    try:
        ...
        model = AutoModelForCausalLM.from_pretrained(
            ...
            device_map="auto",
            attn_implementation="sdpa",   # O(seq) attention memory — avoids the
                                          # eager O(seq^2) score matrix at seq=3072
        )
...
def run_inference(val_items):
    ...
    try:
        ...
        for i, item in enumerate(val_items, 1):
            ...
            if not validated:
                validated = keyword_fallback(question, schema)
                log(f"  qid={qid} [{db_id}] empty model output → keyword fallback {validated}")
...
def main():
    ...
    with open(LOG_FILE, 'w') as f:
        f.write(f"test23.py  started {_ts()}\n")
        f.write(f"schema=pkfk  hardware=RTX-4070-12GB  bf16-no-quant  CED-v3  max_length=3072\n\n")

    log_sep("test23.py  —  long-schema fix (max_length=3072)  —  RTX 4070")
