"""
test17.py -- QLoRA (4-bit) experiments on RTX 4070 (12 GB VRAM).

Motivation: test14 revealed a precision/recall trade-off tied to lora_alpha:
  alpha=16 → Table Precision 0.5176 (best ever) but Recall 0.4645
  alpha=32 → Table Recall 0.5083 but Precision drops to 0.4587

test17 probes the middle ground:
  test17-a  alpha=24  lr=2e-4   (intermediate alpha to balance P and R)
  test17-b  alpha=16  lr=1e-4   (alpha=16 kept; lower lr to recover recall)

Also: test14-b showed severe repeated columns → deduplication added to
post-processing.

Baseline config (shared by both experiments):
  Qwen3-1.7B  QLoRA 4-bit  schema_sorted  aug_v2
  r=16  max_length=2048  3 epochs
  batch_size=1  grad_accum=8  completion_only_loss=True  enable_thinking=False

Adapters  → ./adapters/test17-{a,b}/
Preds     → test17-{a,b}-preds.json
Log       → test17-log.txt

Usage (inside tmux on RTX 4070 machine):
    conda run -n cse234 python test17.py
"""
...
LOG_FILE       = './test17-log.txt'

LORA_TARGETS   = ["q_proj", "k_proj", "v_proj", "o_proj"]
BASE_MODEL     = 'Qwen/Qwen3-1.7B'
MAX_NEW_TOKENS = 512

# ── 4-bit quantisation config (shared by training and inference) ──────────────
...
EXPERIMENTS = [
    {
        'tag':          'test17-a',
        'adapter_dir':  './adapters/test17-a',
        'preds_file':   './test17-a-preds.json',
        'lora_r':       16,
        'lora_alpha':   24,    # intermediate between 16 (best P) and 32 (best R)
        'lora_dropout': 0.05,
        'lr':           2e-4,
        'epochs':       3,
        'batch_size':   1,
        'grad_accum':   8,
        'max_length':   2048,
        'hypothesis':   'intermediate alpha balances precision and recall',
    },
    {
        'tag':          'test17-b',
        'adapter_dir':  './adapters/test17-b',
        'preds_file':   './test17-b-preds.json',
        'lora_r':       16,
        'lora_alpha':   16,    # best precision config; lower lr to recover recall
        'lora_dropout': 0.05,
        'lr':           1e-4,
        'epochs':       3,
        'batch_size':   1,
        'grad_accum':   8,
        'max_length':   2048,
        'hypothesis':   'alpha=16 best precision; lower lr may recover recall',
    },
]
...
def run_training(exp: dict, train_data: list) -> bool:
    ...
    log(f"  QLoRA 4-bit (nf4 + double-quant + bf16 compute)")
    log(f"  r={exp['lora_r']}  alpha={exp['lora_alpha']}  dropout={exp['lora_dropout']}")
    log(f"  lr={exp['lr']}  epochs={exp['epochs']}")
    ...
    try:
        ...
        print(f"=== [{tag}] Loading base model with 4-bit quantisation ===", flush=True)
        ...
        print(f"=== [{tag}] prepare_model_for_kbit_training ===", flush=True)
...
def run_inference(exp: dict, val_items: list) -> bool:
    """Load the 4-bit quantised base + LoRA adapter and predict schema links."""
    tag = exp['tag']
    ...
    try:
        ...
        print(f"=== [{tag}] Loading 4-bit base model + adapter ===", flush=True)
...
def main():
    with open(LOG_FILE, 'w') as f:
        f.write(f"test17.py  started {_ts()}\n")
        f.write(f"base_model={BASE_MODEL}  quant=4bit-nf4  schema=schema_sorted\n")
        f.write(f"train={TRAIN_JSON}  hardware=RTX4070-12GB\n\n")
        for exp in EXPERIMENTS:
            f.write(
                f"  {exp['tag']}: alpha={exp['lora_alpha']}  lr={exp['lr']}  "
                f"dropout={exp['lora_dropout']}  epochs={exp['epochs']}\n"
                f"    hypothesis: {exp['hypothesis']}\n"
            )
        f.write('\n')

    log_sep("test17.py  —  RTX 4070 12GB  —  QLoRA 4-bit  —  alpha/lr sweep")
