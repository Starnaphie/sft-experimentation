"""
test1.py -- 3 sequential SFT experiments on AWS L4 (TRL 1.5.1 / SFTConfig).
Run overnight while RTX 4070 runs test2.py — together they cover 6 distinct configs.

Experiments:
  v2_sorted   Qwen2.5-1.5B  schema_sorted      aug_v2 data  LoRA r=16
  v2_pkfk     Qwen2.5-1.5B  schema_sorted_pkfk aug_v2 data  LoRA r=16  (+ PK/FK hints)
  qwen3       Qwen3-1.7B    schema_sorted      aug_v2 data  LoRA r=16  (newer architecture)

Usage:
    python test1.py                # run all 3 in sequence
    python test1.py v2_sorted      # run single experiment by name

Adapter  → ./adapter_t1_{name}/
Preds    → ./preds_t1_{name}.json
"""
...
EXPERIMENTS = [
    ('v2_sorted', 'Qwen/Qwen2.5-1.5B-Instruct',  'schema_sorted',     16, 32, 2e-4, 3, 2, 2),
    ('v2_pkfk',   'Qwen/Qwen2.5-1.5B-Instruct',  'schema_sorted_pkfk',16, 32, 2e-4, 3, 2, 2),
    ('qwen3',     'Qwen/Qwen3-1.7B',              'schema_sorted',     16, 32, 2e-4, 3, 2, 2),
]
...
def run_experiment(name, base_model, schema_format, lora_r, lora_alpha,
    ...
    adapter_dir = f"./adapter_t1_{name}"
    print(f"\n{'='*65}")
...
def run_eval(adapter_dir, schema_format, name):
    preds_file = f"./preds_t1_{name}.json"
...
def main():
    ...
    print(f"  cp -r ./adapter_t1_{best}/* ./adapter/")
