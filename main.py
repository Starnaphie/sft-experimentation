...
# ── Recall post-processing (no retraining) ────────────────────────────────────
# The leaderboard metric is set-based (P+R+F1)/3, which rewards recall: an
# over-prediction that contains the gold links gets partial credit, while a
# precise-but-wrong single pick scores 0. The fine-tuned model tends to
# over-predict TABLES but UNDER-predict COLUMNS. This deterministic step lifts
# column (and a little table) recall on the model's existing output — no model
# change. Simulated offline: test19-a 0.4427→0.4656, test20-a 0.4990→0.5152.
...
def postprocess_links(validated: dict, question: str, schema: dict,
    ...
    """Recall-boosting post-processing. `schema` is the full {table: [cols]}."""
    qt = _pp_tok(question)
...
def predict_schema_links(question: str, db_id: str, schemas_dir: str,
    ...
    if predictor is not None:
        ...
        # Recall post-processing on the full schema (no retraining)
        if postprocess:
            result = postprocess_links(result, question, schema)
...
def main():
    ...
    ap.add_argument('--schema_format', default='schema_sorted_pkfk',
    ...
    ap.add_argument('--no_postprocess', action='store_true',
                    help='Disable recall post-processing (submit raw model output only)')
    args = ap.parse_args()
    ...
    if _adapter_ready(args.adapter_dir):
        print(f"[mode] fine-tuned model  (schema_format={args.schema_format}, "
              f"hybrid={hybrid}, postprocess={postproc})")
    ...
    for i, it in enumerate(items, 1):
        ...
        links = predict_schema_links(
            ...
            debug=debug_this, hybrid_fallback=hybrid, postprocess=postproc)
        if debug_this:
