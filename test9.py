"""
test9.py -- Two-stage output format: model commits to tables before columns.

Hypothesis: explicitly predicting tables first acts as chain-of-thought,
improving column precision on the second line.

Training completion format:
    Tables: ["Table1", "Table2"]
    {"Table1": ["col_a", "col_b"], "Table2": ["col_c"]}

Experiment:
  qwen3_2stage  Qwen3-1.7B  schema_sorted_2stage  aug_v2 data  LoRA r=16

Baseline to beat: Leaderboard=0.4415  Table=0.5538  Column=0.3292

Usage:
    python test9.py                  # run the single experiment
    python test9.py qwen3_2stage     # same

Adapter  → ./adapter_t9_{name}/
Preds    → ./preds_t9_{name}.json
"""
...
EXPERIMENTS = [
    ('qwen3_2stage', 'Qwen/Qwen3-1.7B', 'schema_sorted_2stage', 16, 32, 2e-4, 3, 2, 2),
]
...
def serialize(schema, fmt, col_ann):
    ...
    if fmt in ('schema_sorted', 'schema_sorted_pkfk', 'schema_sorted_2stage'):
        # schema_sorted_2stage uses same schema serialization as schema_sorted
        lines = []
...
def build_dataset(data, tokenizer, schema_format, schemas_dir, base_model):
    ...
    if schema_format == 'schema_sorted_pkfk':
        sys_prompt = SYSTEM_PROMPT_PKFK
    elif schema_format == 'schema_sorted_2stage':
        sys_prompt = SYSTEM_PROMPT_2STAGE
    else:
    ...
    for item in data:
        ...
        if schema_format == 'schema_sorted_2stage':
            gold_tables = list(answer.keys())
            tables_line = json.dumps(gold_tables, ensure_ascii=False)
            links_line  = json.dumps(answer, ensure_ascii=False)
            assistant_content = f"Tables: {tables_line}\n{links_line}"
        else:
...
def run_experiment(name, base_model, schema_format, lora_r, lora_alpha,
    ...
    adapter_dir = f"./adapter_t9_{name}"
    print(f"\n{'='*65}")
...
def run_eval(adapter_dir, base_model, schema_format, name):
    preds_file = f"./preds_t9_{name}.json"
...
def main():
    ...
    print(f"  cp -r ./adapter_t9_{best}/* ./adapter/")
