"""
test11.py -- Effect of longer context window + balanced LoRA scaling.

Changes from Method 3 baseline:
  max_length=2048     (baseline uses 1024) — fits longer schemas without truncation
  lora_alpha=16       (baseline uses 32)   — alpha==r is the standard "no scaling" setting

Everything else identical to Method 3:
  Qwen3-1.7B + schema_sorted + aug_v2 + lr=2e-4 + r=16 + 3 epochs
  completion_only_loss=True

Baseline to beat: Leaderboard=0.4415  Table=0.5538  Column=0.3292

Usage:
    python test11.py                 # run the single experiment
    python test11.py qwen3_maxlen    # same

Adapter  → ./adapter_t11_{name}/
Preds    → ./preds_t11_{name}.json
"""
...
EXPERIMENTS = [
    ('qwen3_maxlen', 'Qwen/Qwen3-1.7B', 'schema_sorted', 16, 16, 2e-4, 3, 2, 2),
    #                                                          ^^  lora_alpha=16 (== r)
]
...
def run_experiment(name, base_model, schema_format, lora_r, lora_alpha,
    ...
    sft_config = SFTConfig(
        ...
        max_length=2048,              # ← KEY CHANGE 1 (baseline uses 1024)
        completion_only_loss=True,
        packing=False,
    )
...
def run_eval(adapter_dir, base_model, schema_format, name):
    preds_file = f"./preds_t11_{name}.json"
...
def main():
    ...
    print(f"  cp -r ./adapter_t11_{best}/* ./adapter/")
